...
if REGISTERED_USERS.get(user_name) == password:
    ...
    selection_of_text = input("Enter a number btw. 1 and 3 to select: ")
    print("-"*40)
    ...
    if not selection_of_text.isdigit():
        ...
        exit()
    elif not (1 <= int(selection_of_text) <= 3):
    ...
    print("There are",uppersace_words, "uppercase words.")
    ...
    print("-" * 40)
    print(f"{'LEN':>3} | {'OCCURRENCES'.ljust(bar_width)} | {'NR.':>3}")
    print("-" * 40)

    for i in range(1, max_len + 1):
        count = number_of_words.count(i)  
        if count > 0:  
            print(f"{i:>3} | {'*' * count:<{bar_width}} | {count:>3}")
